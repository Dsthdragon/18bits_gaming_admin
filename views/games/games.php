			...
			<div class="uk-card-body">
				<div class="uk-grid">
					<div class="uk-width-1-1@s uk-width-1-3@m">
						...
						<a href="<?= URL ?>games" class="uk-button-flat <?= ($this->type != "published" && $this->type != "top") ? "" : "blue-text" ?>">All</a> | 
						<a href="<?= URL ?>games/published" class="uk-button-flat <?= ($this->type == "published") ? "" : "blue-text" ?>">Published</a> | 
						<a href="<?= URL ?>games/draft" class="uk-button-flat <?= ($this->type == "draft") ? "" : "blue-text" ?>">Draft</a>
					</div>
				...
				<div class="uk-grid">
					<table class="uk-table">
						<tbody>
							<?php
							$data = paginator::paginate($this->games, 10, $this->currentPage);
							$count = $data[1];
							$data = $data[0];
							?>
							<?php foreach($data  as $key => $value): ?>
								<tr>
									<td>
										<p>
											<label>
												<input type="checkbox" class="form-control myCheckBoxes" name="games[]" value="<?= $value['id']; ?>" form="editAll">
												<span></span>
											</label>
										</p>
									</td>
									<td>
										<?= $value['title']; ?><hr />
										<a href="<?= URL; ?>game/<?= $value['id'] ?>" class="uk-button blue white-text">view</a>
						...
						<tfoot>
							<tr>
								<td colspan="6" class="center-align">
									<?php paginator::view($count, $this->currentPage, "games", $this->type); ?>
								</td>
							</tr>
						</tfoot>
					</table>
